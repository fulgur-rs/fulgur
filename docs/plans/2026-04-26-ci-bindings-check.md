...
**Architecture:** 単一 job (Linux only) を `oxidize-rb/actions/setup-ruby-and-rust` で Ruby+Rust 環境を整備し、pyfulgur (extension-module) と fulgur-ruby (ruby-api) の 2 crate に対して `cargo check` と `cargo clippy --all-targets -- -D warnings` を順に実行する。matrix なし。
...
```yaml
  ...
  bindings-check:
    ...
    steps:
      ...
      - name: cargo check pyfulgur (extension-module)
        run: cargo check -p pyfulgur --features extension-module
      - name: cargo check fulgur-ruby (ruby-api)
        run: cargo check -p fulgur-ruby --features ruby-api
...
Run: `cargo check -p pyfulgur --features extension-module 2>&1 | tail -5`
Expected: `Finished` で終わる (現在の main は PR #217 で修正済み)。

Run: `cargo check -p fulgur-ruby --features ruby-api 2>&1 | tail -5`
...
2. ✅ `cargo check -p pyfulgur --features extension-module` が CI で実行される
3. ✅ `cargo check -p fulgur-ruby --features ruby-api` が CI で実行される
4. ✅ それぞれに対して `cargo clippy ... --all-targets -- -D warnings` も実行される
5. ✅ PR の CI で job が green
6. ✅ 既存 jobs に変更なし
